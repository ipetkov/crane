...
let
  ...
  removeReferencesToRustToolchain =
    let
      myLibFatToolchain = myLib.overrideToolchain (
        p:
    ...
    in
    checkRefs "test_removeReferencesToRustToolchain" (
      myLibFatToolchain.buildPackage {
  ...
  # addresses https://github.com/ipetkov/crane/issues/773
  removeReferencesToRustToolchainFenix = checkRefs "test_removeReferencesToRustToolchainFenix" (
    myLibFenix.buildPackage {
